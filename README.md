...
Another attempt to experiment with Metal shader is to make a standalone macOS app, **MetalBox**, and to run that in debugger mode. It needs to be a macOS app and not a iOS app, because the iOS Simulator cannot simulate metal.

### MetalBox debugging

The usefulness of a macOS app, is that you can _Capture the GPU Frame_ while running in debug mode in Xcode.
